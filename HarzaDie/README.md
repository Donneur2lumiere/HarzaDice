...
```bash
python3 HarzaDiceApp.py
```

- Cliquer (clic gauche) sur un dé de couleur pour incrémenter le nombre de dés de cette couleur à lancer.
- Couleurs disponibles : Rouge, Orange, Noir, Vert, Bleu.
- Choisir le type d'arme (Courte / Moyenne / Distante) en haut de la fenêtre, nécessaire pour le calcul des cas critiques.
- Cliquer **LANCER !** pour lancer les dés sélectionnés et afficher le résultat (total chiffré + détail + éventuelle réussite/échec critique).
- Re-cliquer **LANCER !** efface uniquement les résultats et relance exactement le même nombre de dés qu'auparavant.
- Cliquer **Oublie...** efface les résultats et remet à zéro la sélection des dés.

Les symboles ★ (critique) et ☠ (fumble) sont utilisés pour l'affichage.

L'ancien bot Discord est conservé dans `HazaDice.py` et `HazaDice_V2.py`.

## utilisation (bot Discord, ancien)
lancer HazaDice.py
